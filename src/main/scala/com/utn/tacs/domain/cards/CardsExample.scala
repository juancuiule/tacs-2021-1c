...
//
//sealed trait CardServiceError
//case object ApiResponseParseError extends CardServiceError
//case object ApiSuperheroNotFoundError extends CardServiceError
//case object ApiBadNameSearchRequest extends CardServiceError
//case object ApiUnknownError extends CardServiceError
//
...
//
//  def pruebaTry(algo: String): Try[Card] = {
//    print(algo)
//    Failure(AnotherCardError())
//  }
//
...
//  def getCardsFromAPI(searchName: String)(implicit FF: Sync[F]): F[Either[CardServiceError, SearchResponse]] = {
...
//        r.attemptAs[SearchResponse].leftFlatMap[SearchResponse, CardServiceError](_ => EitherT {
...
//          r.attemptAs[ApiResponseError].map {
//            case ApiResponseError("error", "character with given name not found") => ApiSuperheroNotFoundError
//            case ApiResponseError("error", "bad name search request") => ApiBadNameSearchRequest
//            case _ => ApiUnknownError
//          }.leftMap(_ => ApiResponseParseError).value.map({
//            case Right(x) => Left(x)
//            case Left(x) => Left(x)
//          })
//        })
//      }.value
//      case _ => Either.left[CardServiceError, SearchResponse](ApiUnknownError).pure[F]
//    }
//  }
//}
//
...
//
//
//case class CardNotFound() extends Throwable
//
//case class AnotherCardError() extends Throwable
//
////
//
//// Endpoint
...
//          case Left(ApiResponseParseError) => InternalServerError("parse error")
//          case Left(ApiSuperheroNotFoundError) => NotFound(s"$searchName not found")
//          case Left(ApiBadNameSearchRequest) => InternalServerError("bad name search")
//          case Left(ApiUnknownError) => InternalServerError()
//        }
//    }
//
//  def prueba: HttpRoutes[F] =
//    HttpRoutes.of[F] {
//      case GET -> Root / "error" =>
//        val action = service.pruebaTry("Algo")
//        action match {
//          case Success(card) => Ok(card.asJson)
//          case Failure(_: CardNotFound) => NotFound("card not found")
//          case Failure(_: AnotherCardError) => InternalServerError("another card error")
//          case Failure(_) => InternalServerError("no se que paso")
//        }
//    }
//
