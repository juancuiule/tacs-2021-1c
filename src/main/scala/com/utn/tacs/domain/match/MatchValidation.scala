...
class MatchValidation[F[_] : Applicative](repository: MatchRepository) {
  def doesNotExist(_match: Match): EitherT[F, MatchAlreadyExistsError, Unit] = EitherT.fromEither {
    repository.getMatch(_match.matchId).fold[Either[MatchAlreadyExistsError, Unit]](Right(()))(m => Left(MatchAlreadyExistsError(m)))
  }
